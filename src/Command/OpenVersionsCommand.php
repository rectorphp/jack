...
final class OpenVersionsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $this->symfonyStyle->warning(\sprintf('Found %d outdated package%s', $outdatedComposer->count(), $outdatedComposer->count() === 1 ? '' : 's'));
        $this->symfonyStyle->listing([\sprintf('%d prod package%s', $outdatedComposer->getProdPackagesCount(), $outdatedComposer->getProdPackagesCount() === 1 ? '' : 's'), \sprintf('%d dev package%s', $outdatedComposer->getDevPackagesCount(), $outdatedComposer->getDevPackagesCount() === 1 ? '' : 's')]);
