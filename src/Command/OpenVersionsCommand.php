...
final class OpenVersionsCommand extends Command
{
    ...
    public function __construct(OutdatedComposerFactory $outdatedComposerFactory, ComposerOutdatedResponseProvider $composerOutdatedResponseProvider, OpenVersionsComposerProcessor $openVersionsComposerProcessor)
    {
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $symfonyStyle->writeln(\sprintf('Found <fg=yellow>%d outdated package%s</>', $outdatedComposer->count(), $outdatedComposer->count() === 1 ? '' : 's'));
        $symfonyStyle->writeln(\sprintf(' * %d prod package%s', $outdatedComposer->getProdPackagesCount(), $outdatedComposer->getProdPackagesCount() === 1 ? '' : 's'));
        $symfonyStyle->writeln(\sprintf(' * %d dev package%s', $outdatedComposer->getDevPackagesCount(), $outdatedComposer->getDevPackagesCount() === 1 ? '' : 's'));
        $symfonyStyle->newLine();
        ...
        $changedPackageVersionsResult = $this->openVersionsComposerProcessor->process($composerJsonContents, $outdatedComposer, $limit, $onlyDev, $packagePrefix);
        ...
        $symfonyStyle->success(\sprintf('%d packages %s opened up to the next nearest version.%s%s "composer update" to push versions up', \count($openedPackages), $isDryRun ? 'would be (is "--dry-run")' : 'were', \PHP_EOL, $isDryRun ? 'Then you would run' : 'Now run'));
        foreach ($openedPackages as $openedPackage) {
            $symfonyStyle->writeln(\sprintf(' * Opened "<fg=green>%s</>" package to "<fg=yellow>%s</>" version', $openedPackage->getPackageName(), $openedPackage->getNewVersion()));
        }
        return self::SUCCESS;
    }
}
